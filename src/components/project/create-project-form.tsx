...
export function CreateProjectForm({ onboarding = false }: { onboarding?: boolean }) {
  ...
  return <form action={action} className="space-y-6" aria-busy={pending}>
    ...
    <div className="flex items-center justify-end gap-3 border-t border-border pt-5">
      {!onboarding && (pending ? <span className="px-4 text-sm text-muted">Cancel</span> : <Link href="/app/projects" className="rounded-md px-4 py-2 text-sm text-secondary hover:bg-hover">Cancel</Link>)}
      <Button type="submit" disabled={pending} className="h-11 w-full sm:w-auto">{pending ? <LoaderCircle className="animate-spin" /> : <ArrowRight />}{pending ? "Creating project…" : "Create project"}</Button>
    </div>
  </form>;
}
