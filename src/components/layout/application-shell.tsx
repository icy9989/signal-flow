...
export function ApplicationShell({ children, workspaces, activeWorkspace, projectSelection }: { children: React.ReactNode; workspaces: Workspace[]; activeWorkspace?: Workspace; projectSelection: ProjectSelectorProps }) {
  return <div className="min-h-dvh">
    ...
    <aside className="fixed inset-y-0 left-0 hidden w-[248px] flex-col overflow-y-auto border-r border-border bg-sidebar px-4 py-6 lg:flex">
      ...
      {activeWorkspace && <ProjectSelector key={`${activeWorkspace.id}:${projectSelection.activeId}`} {...projectSelection} />}
      <Navigation />
    ...
    <div className="lg:pl-[248px]">
      <header className="flex h-16 items-center justify-between gap-3 border-b border-border px-4 sm:px-8">
        <div className="flex min-w-0 items-center gap-3"><MobileNavigation workspaces={workspaces} activeId={activeWorkspace?.id} projectSelection={projectSelection} /><span className="truncate text-sm text-secondary">{activeWorkspace?.name ?? "Workspace setup"}</span></div>
