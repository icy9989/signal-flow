...
export function CreateWorkspaceForm() {
  ...
  return <form action={action} className="space-y-6" aria-busy={pending}>
    ...
    <div className="flex flex-wrap items-center justify-end gap-3 border-t border-border pt-5">
      {pending ? <span className="px-4 text-sm text-muted">Cancel</span> : <Link href="/app/overview" className="rounded-md px-4 py-2 text-sm text-secondary hover:bg-hover hover:text-foreground">Cancel</Link>}
      <Button type="submit" disabled={pending} className="h-10">{pending ? <LoaderCircle className="animate-spin" /> : null}{pending ? "Creating workspace…" : "Create workspace"}{!pending && <ArrowRight />}</Button>
    </div>
  </form>;
}
